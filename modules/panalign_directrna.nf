...
process PANALIGN_DIRECTRNA {
	...
	when:
	!params.panalign_off && params.process_rna && params.process_nanopore
	
	script:
        """
	(bowtie2 -q -x ${pangenome_path}/${params.pangenome} -U ${reads_file} --un-gz "${sample_id}_bt2_pangenome_unaligned.fastq.gz" \\
